...
def test_parse_lit_any_lit():
    assert BetterParser().parse("abc*xyz") == Lit("abc", Any(Lit("xyz")))


def test_parse_either_two_lit():
    assert BetterParser().parse("{abc,def}") == Either([Lit("abc"), Lit("def")])


def test_parse_any_either():
    assert BetterParser().parse("*{abc,def}") == Any(Either([Lit("abc"), Lit("def")]))
